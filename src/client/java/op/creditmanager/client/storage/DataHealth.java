...
public final class DataHealth {
    private static final AtomicReference<String> pendingWarning = new AtomicReference<>();
    ...
    public static synchronized void reportRecoveryRequired(Path path, boolean backupCreated) {
        ...
        if (pendingWarning.get() == null) {
            pendingWarning.compareAndSet(null, backupCreated
    ...
    public static synchronized void reportRecoveryRequired(String reason) {
        lastCheckedAt = System.currentTimeMillis();
        reasons.add(reason);
        pendingWarning.compareAndSet(null,
                "Ungültige Daten erkannt. Änderungen sind bis zur Prüfung gesperrt; bitte das Backup prüfen.");
    }

    public static synchronized void reportWarning(String reason, String warning) {
        lastCheckedAt = System.currentTimeMillis();
        reasons.add(reason);
        pendingWarning.compareAndSet(null, warning);
    }
    ...
    public static String consumeWarning() { return pendingWarning.getAndSet(null); }
    ...
    public static synchronized void clearReasons() { reasons.clear(); pendingWarning.set(null); lastCheckedAt = System.currentTimeMillis(); }
}
